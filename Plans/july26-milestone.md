...
- [x] Compound index `{ visitorId: 1, createdAt: -1 }` on `BookingRequest` — flagged as a follow-up when the guest-quota tiering shipped (2026-07-04); added 2026-07-11 (`ae4471e`), along with the same index on `Application` for its identical unindexed quota scan.
