...
| `POST /signup` | — | Inline signup, embedded on both `/hire/success` and `/track` (2026-07-11) — creates a `User`, links the relevant booking via `crCode`, logs in; redirects back to whichever page it was submitted from via a whitelisted `returnTo` field. If the booking is already linked to an account (`clientId` set), redirects to `/login?cr=` instead of processing signup at all (2026-07-12, uncommitted) — prevents the same booking being repeatedly re-signed-up-for with fresh emails to hijack it and stack extra discount. Grants a one-time 15%-off "welcome discount" (`User.discountPercent`/`discountExpiresAt`, 15-day window, `ce11320`) to genuinely first-time emails only — an email with *any* prior `BookingRequest` under it, paid or not, account-linked or not, declined or not, doesn't get a second discount (`4db3d62`, broadened 2026-07-12, uncommitted) — normally redeemable on a later `/hire` submission, but if the triggering booking hasn't been priced yet (`!agreedPrice`) the discount is applied to it directly instead (retroactive application, `bddbf7f`), which is what actually makes it an acquisition incentive rather than a reward for some hypothetical next project. An hourly job (`lib/discountExpiry.js`) emails + in-app-notifies accounts whose unused discount is expiring within 3 days |
